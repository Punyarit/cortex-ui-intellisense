...
function provideStateCompletionItems(attrValue) {
    ...
    if (words.length === 1) {
        ...
        return propertyNames.map((name) => new vscode.CompletionItem(name, vscode.CompletionItemKind.Text));
    }
        ...
        if (propertyValues) {
            return propertyValues.map((value) => new vscode.CompletionItem(value, vscode.CompletionItemKind.Value));
        }
    }
